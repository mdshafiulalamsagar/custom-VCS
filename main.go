...
func main() {
	...
	case "add":
		if len(os.Args) < 3 {
			fmt.Println("Usage: sagargit add <file_path> or .")
			os.Exit(1)
		}
...
func doCommit(message string) {
	// 1. Load the staging area to enforce the 2-step workflow
	idx := loadIndex()
	
	if len(idx.Entries) == 0 {
		fmt.Println("Nothing to commit! Staging area is empty.")
		fmt.Println("Use 'sagargit add .' to stage files before committing.")
		os.Exit(1)
	}

	// 2. Generate Tree using the original working function
	ignored := loadIgnoreList()
	treeHash := writeTree(".", ignored)

	// 3. Read parent commit hash from the 'main' branch
	parentHash := ""
	branchData, err := os.ReadFile(".sagargit/refs/heads/main")
	if err == nil {
		parentHash = strings.TrimSpace(string(branchData))
	}

	// 4. Create the actual commit object
	commitHash := commitTree(treeHash, parentHash, message)

	// 5. Update Branch Ref and HEAD to maintain standard Git architecture
	os.MkdirAll(".sagargit/refs/heads", 0755)
	os.WriteFile(".sagargit/refs/heads/main", []byte(commitHash), 0644)
	os.WriteFile(".sagargit/HEAD", []byte("ref: refs/heads/main\n"), 0644)

	// 6. Clear the staging area after a successful commit
	emptyIndex := Index{Entries: make(map[string]string)}
...
// Index struct represents the staging area
type Index struct {
	Entries map[string]string `json:"entries"` // Format: {"file_path": "blob_hash"}
}

// loadIndex reads the current staging area from .sagargit/index
func loadIndex() Index {
	idx := Index{Entries: make(map[string]string)}
	data, err := os.ReadFile(".sagargit/index")
	if err == nil {
...
// saveIndex writes the updated staging area back to .sagargit/index
func saveIndex(idx Index) {
	data, _ := json.MarshalIndent(idx, "", "  ")
	os.WriteFile(".sagargit/index", data, 0644)
}

// doAdd hashes files and adds them to the staging area (index)
func doAdd(targetPath string) {
	ignored := loadIgnoreList()
	idx := loadIndex()
	...
	err := filepath.WalkDir(targetPath, func(path string, d os.DirEntry, err error) error {
		...
		// Skip .sagargit folder and anything in .ignore
		if d.IsDir() {
			if path == ".sagargit" || ignored[path] || ignored[d.Name()] {
				return filepath.SkipDir
			}
			return nil
		}
		if ignored[path] || ignored[d.Name()] {
			return nil
		}

		// Create blob for the file and save its hash in the index
		hash := writeBlob(path)
	...
	saveIndex(idx)
	fmt.Printf("Added changes to the staging area.\n")
}

// writeTreeFromIndex generates a Tree object strictly from the Staging Area
func writeTreeFromIndex(idx Index) string {
	var treeContent string
	for path, hash := range idx.Entries {
		treeContent += fmt.Sprintf("blob %s %s\n", hash, path)
	}
	
	// Temporarily write to a file so we can reuse your existing writeBlob function
	tempFile := filepath.Join(".sagargit", "temp_tree")
	os.WriteFile(tempFile, []byte(treeContent), 0644)
	defer os.Remove(tempFile) // Auto cleanup
	
	return writeBlob(tempFile)
}